...
- Le champ `task_details` contient des informations spécifiques telles que la concaténation des valeurs de `event_section` (pour indiquer la catégorie de l'événement) et d'autres indicateurs/références comme `supplierName` (nom du fournisseur) ou d'autres éléments contextuels.
